...
def runtests(options=None):
    ...
    failures = test_runner.run_tests(labels or ['docdata'])
    sys.exit(failures)


if __name__ == '__main__':
    parser = ArgumentParser(description="Run the django-docdata test suite.")
    parser.add_argument('labels', nargs='*', metavar='label',
        help='Optional path(s) to test modules; e.g. "docdata.tests.OfflinePaymentTests".')
    parser.add_argument(
